  ...
  if (step === "boat" && selDate) return (
    <div style={{ flex: 1, padding: "0 20px 40px", maxWidth: 540, margin: "0 auto", width: "100%", boxSizing: "border-box" }}>
      ...
      <div style={{ background: "#fff", borderRadius: 20, padding: 24 }}>
        ...
        {(() => {
          const allBk = selDate.boats.flatMap(boat =>
            boat.bookings.map(bk => ({ ...bk, boat }))
          );
          if (allBk.length === 0) return null;
          return (
            <div style={{ marginTop: 20 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: "rgba(255,255,255,0.7)", textTransform: "uppercase", letterSpacing: 1, marginBottom: 10 }}>
                Réservations du jour ({allBk.length})
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {allBk.map((bk, idx) => {
                  ...
                  const pax      = bk.adults + (bk.children || 0);
                  const tel      = fullPhone(bk);
                  const waNum    = tel ? tel.replace(/[^0-9]/g, "") : null;
                  const boatIcon = bk.boat.name === "Aloes Vera" ? "🛥️" : "🚤";

                  return (
                    <div key={idx} style={{ background: "rgba(255,255,255,0.12)", borderRadius: 12, padding: "12px 14px", backdropFilter: "blur(4px)" }}>
                      <Row style={{ marginBottom: 6, gap: 8, flexWrap: "wrap" }}>
                        <span style={{ background: srcColor, color: "#fff", fontSize: 10, padding: "2px 8px", borderRadius: 7, fontWeight: 700, flexShrink: 0 }}>
                          {isWoo ? "🌐 Web" : srcLabel}
                        </span>
                        <span style={{ fontSize: 11, color: "rgba(255,255,255,0.6)" }}>{boatIcon}</span>
                        <span style={{ fontWeight: 700, color: "#fff", fontSize: 13, flex: 1 }}>{bk.name}</span>
                        <span style={{ fontSize: 12, color: "rgba(255,255,255,0.75)", fontWeight: 600, flexShrink: 0 }}>
                      ...
                      {bk.phone && (
                        <Row gap={8} style={{ marginTop: 6 }}>
                          <a href={`https://wa.me/${waNum}`} target="_blank" rel="noreferrer"
                            style={{ display: "flex", alignItems: "center", gap: 4, background: "#25D366", color: "#fff", borderRadius: 7, padding: "5px 12px", textDecoration: "none", fontSize: 11, fontWeight: 700 }}>
                          ...
                          <a href={`tel:${tel}`}
                            style={{ display: "flex", alignItems: "center", gap: 4, background: "rgba(255,255,255,0.2)", color: "#fff", borderRadius: 7, padding: "5px 12px", textDecoration: "none", fontSize: 11, fontWeight: 700 }}>
                            📞 Appeler
                          </a>
                        </Row>
                      )}
